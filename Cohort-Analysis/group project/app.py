...
def build_demo_world(cfg: Config) -> dict:
    ...
    while day <= end:
        ...
        if day == big_day:
            cands = [c for c in ("SKU-1002", "SKU-1001", "SKU-1000", "SKU-1006")
                     if c in qty and qty[c] >= 100] or [c for c in skus if qty[c] >= 60]
            if cands:
                c = cands[0]
    ...
    gl_overrides = {adjustments[-1].id: 2.0} if adjustments else {}
...
def replay_stock_ledger(skus, txns) -> dict:
    ...
    for t in sorted(txns, key=lambda x: (x.date, x.id)):
        if t.ttype == "receipt":
            ...
            q = abs(t.qty)          # units leaving/entering stock
            v = q * avg             # value at moving-average cost
            per_txn_value[t.id] = v
            if t.qty < 0:
                qty[t.sku] -= q
                value[t.sku] -= v
